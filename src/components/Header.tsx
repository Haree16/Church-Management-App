...
export const Header: React.FC<HeaderProps> = ({
  ...
  const churchNotifications = safeNotifications.filter(
    (n) => (n.church_id || n.churchId) === currentChurch?.id || (!n.church_id && !n.churchId)
  );
  const unreadNotifsCount = churchNotifications.filter((n) => !n.read).length;
                      ...
                      <div className="space-y-2.5">
                        {churchNotifications.slice(0, 10).map((notif) => (
                          <div
                            key={notif.id}
                            onClick={() => {
                              if (!notif.read && onMarkNotifRead) onMarkNotifRead(notif.id);
                          ...
                          >
                            <div className="flex items-center justify-between gap-2">
                              <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-md border ${
                                ...
                                notif.category === 'Event' ? 'bg-amber-100 text-amber-900 border-amber-200' :
                    ...
                    <div className="bg-slate-50 p-4 border-t border-slate-200 flex items-center justify-between gap-3 shrink-0">
                      {onClearAllNotifs ? (
                        <button
                          ...
                          className="text-xs text-slate-500 hover:text-rose-600 flex items-center gap-1.5 font-bold transition px-3 py-2 rounded-xl hover:bg-rose-50 border border-transparent hover:border-rose-200"
                        >
